...
class DD4AO:
    ...
    def build_problem(self):
        ...
        # Pc depends only on Parameters (X_c, Y_c) -> purely numeric each iteration.
        # Stash the expression so solve_iter can pull .value from it cheaply.
        self.Pc_expr = cp.multiply(self.G_freq, Xc) + Yc
        P = cp.multiply(self.G_freq, Xf) + Yf

        # |Pc|^2 is a nonlinear function of parameters only (no Variable) -> risks
        # breaking DPP. Since it's purely numeric anyway, make it its own Parameter,
        # refreshed via numpy each iteration (same trick as normal-direction below).
        ...
        print('DPP compliant:', self.prob.is_dcp(dpp=True))  # sanity check, remove later
    ...
    def _build_robust_nyquist(self, P):
        ...
        self.Pc_ext = cp.vstack((Pc_start, Pc, Pc_end))   # stash: parameter-only, needed each iter
        Cp2 = cp.vstack([P_start, P, P_end])

        # self.n_nyquist = cp.Parameter(self.Pc_ext.shape, complex=True)
    ...
    def _build_con_radius(self, z_, szy, Y_n):
        ...
        self.Zys_ = Zys_  # stash: fixed matrix, needed each iteration to recompute Ycs_ in numpy

        # self.n_radius = cp.Parameter((Zys_.shape[0], 1), complex=True)
    ...
    def solve_iter(self, verbose):
        # refresh everything that's purely a function of the current Parameters
